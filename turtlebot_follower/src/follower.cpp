/*
 This code is written by Miguel Nabatilan and is a modified version of the turtlebot_follower
 demo code found in the ROS tutorials. This code should run to detect and follow an intruder.
 */
...
int i; //initialize counter as global variable for calling robotsound topic
...
namespace turtlebot_follower
    ...
    private:
        ...
        virtual void onInit()
        {
            ...
            robotsoundpub_ = private_nh.advertise<sound_play::SoundRequest>("/robotsound",1); //creating new publisher to topic robotsound
        ...
        // function to publish a sound to topic robotsound
        void robotsound()
        {
            sound_play::SoundRequest play;
            play.sound = -2;
            play.command = 2;
            play.arg = "/home/turtlebot/turtlebot/src/turtlebot_apps/turtlebot_follower/src/Kill Bill Ironside Siren Sound.wav";
            robotsoundpub_.publish(play);
            ROS_INFO ("published to topic robotsound");
        }
        
        // function to publish a stop command to the topic robotsound
        void robotsoundstop()
        {
            sound_play::SoundRequest stop;
            stop.command = 0;
            robotsoundpub_.publish(stop);
            ROS_INFO ("stop publishing to robotsound");
        }
        ...
        {
            geometry_msgs::Twist velmsg; //creating new variable to publish velocity values to Twist
            velmsg.linear.x=0;
            ...
            if (n>4000)
            {
                ...
                y /= n;
                if(z > max_z_){
                    ROS_INFO_THROTTLE(1, "Centroid too far away %f, turning in place\n", z);
                    if (enabled_)
                    {
                        cmdpub_.publish(velmsg);
                        i = 150; //set counter back to 150
                    }
                    robotsoundstop(); //stop playing current sound
                    return;
                }
                ...
                if (enabled_)
                {
                    if (i % 150 == 0) //method to play sound every 150 iterations
                    {
                        robotsound(); //play sound
                    }
                    i++; //increase the counter
                    geometry_msgs::TwistPtr cmd(new geometry_msgs::Twist());
                    cmd->linear.x = (z - goal_z_) * z_scale_;
                    cmd->angular.z = -x * x_scale_;
                    cmdpub_.publish(cmd); //publish velocity commands to kobuki base
                }
            }
            else
            {
                ROS_INFO_THROTTLE(1, "Not enough points(%d) detected, spinning in place", n);
                publishMarker(x, y, z);
                
                if (enabled_)
                {
                    i = 150; //reset counter to 150
                    cmdpub_.publish(velmsg); // command to spin in place
                    robotsoundstop(); // publish stop command to /robotsound
                }
            }
